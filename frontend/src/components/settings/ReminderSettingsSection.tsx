...
export function ReminderSettingsSection() {
  ...
  return (
    <section className="glass-card rounded-lg p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[200px_200px_minmax(0,1fr)]">
        <div className="md:col-span-2 xl:col-span-1">
          <label className="settings-label mb-1.5">提醒开关</label>
          <button
            onClick={() => updateRingtone({ enabled: !settings.ringtone.enabled })}
            className="field-input flex h-9 items-center justify-between gap-3 px-3 text-left"
            aria-label="切换提醒"
          >
            <span className="text-xs text-foreground">
              {settings.ringtone.enabled ? "已开启" : "已关闭"}
            </span>
            <span
              className={cn(
