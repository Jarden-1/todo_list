...
export function StructuredTodoForm({ projects, onCancel, onAddTodo }: StructuredTodoFormProps) {
  ...
  // dueAt holds a resolved ISO string for ALL precisions (datetime exact,
  // day = 23:59, week = Sunday 23:59), kept in sync with duePrecision by the
  // shared DuePrecisionPicker. `none` => dueAt is null.
    ...
    <div className="space-y-3 p-4 pt-14">
      ...
      <div>
        ...
        <DuePrecisionPicker
          ...
          onChange={(next) => {
            setDueAt(next.dueAt);
            setDuePrecision(next.dueAtPrecision);
          }}
        />
      </div>
